...
public class OthelloMinimax extends OthelloPlayer {
	...
	private int evaluate(OthelloState state){
        int score = 0;
        for(int i = 0;i<state.boardSize;i++)
            for(int j = 0;j<state.boardSize;j++) {
                if (state.board[i][j]==state.PLAYER1) score++;
                if (state.board[i][j]==state.PLAYER2) score--;
                //the value of pieces in four edge should be a little bit high because
                //it always lead to some verticak change
                if (i == 0 || i == 7) {
                	if (state.board[i][j]==state.PLAYER1) score++;
                    if (state.board[i][j]==state.PLAYER2) score--;
                }
