...
//                       Mar. 2010 - reformatted #includes and comments
//
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
	...
	int main(void)
	{
		...
		const int MAX_ITERATIONS = 100000000; // not enough - man this thing is slow
		...
		do {
			...
			if( even ) 
				// add zeroth, second, fourth, sixth, etc. terms
				newPartialSum = oldPartialSum + nextTerm;
			else 
				// subtract first, third, fifth, etc. terms
				newPartialSum = oldPartialSum - nextTerm;
