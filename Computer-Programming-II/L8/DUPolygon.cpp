...
DUPolygon& DUPolygon::operator=(DUPolygon &p){
    numPoints_ = p.numPoints();
    closed_polygon_ = p.isClosed();
    first_ = p.first_;
    last_ = p.last_;
    return p;
}
DUPoint& DUPolygon::operator+(DUPoint &d){
    DUPolygonNode* ptr ;
    ptr = new DUPolygonNode(d) ;
    if( last_ != NULL )
        last_->setLink(ptr) ;
    last_ = ptr ;
    if( first_ == NULL )
...
DUPoint& DUPolygon::operator[](int i){
    DUPoint temp=getPoint(i);
    return temp;
}

DUPoint& DUPolygon::operator++(){
   DUPoint temp = first_->getPoint();
    return temp;
}